...
class UploadController {
    ...
    private static function assertCsrf(string $t): void {
        ...
        unset($_SESSION['csrf_tokens'][$t]); // consumir
    }
    ...
    private static function takeReplaceToken(string $tok): ?array {
        ...
        $payload = $bag[$tok];
        unset($_SESSION['replace_tokens'][$tok]); // “una sola vez”
        return $payload;
    }
  ...
  LIMIT 200        ";
    ...
    public static function handle(): string {
        try {
            ...
            // 2) Si algún JSON ya existe por SHA en este usuario -> pedir confirmación de reemplazo
            foreach ($allJsonPaths as $jsonPath) {
                ...
                if ($existing) {
                    ...
                    // Render confirmación y salir (flujo 1 a 1)
                    ob_start(); ?>
                    <!doctype html><html lang="es"><head>
            ...
            // 3) Procesar todos (ningún duplicado)
            $rows = [];
            foreach ($allJsonPaths as $jsonPath) {
    ...
    public static function replace(): string {
        ...
        try {
            // valida que upload a reemplazar sea del usuario
            ...
            // Borra detalle y cabecera (por si no hay ON DELETE CASCADE)
            $del1 = $pdo->prepare("DELETE FROM upload_products WHERE upload_id=?");
            $del1->execute([(int)$payload['replace_upload_id']]);
            $del2 = $pdo->prepare("DELETE FROM uploads WHERE id=?");
            $del2->execute([(int)$payload['replace_upload_id']]);

            // Procesa el JSON nuevo (omite check duplicado)
    ...
    public static function delete(): string {
        ...
        try {
            ...
            if (!$chk->fetchColumn()) {
                $pdo->rollBack();
                return "No autorizado o no existe.";
            }
    ...
    public static function exportUpload(): string {
        ...
        $fname = sprintf(
            "upload_%d_%s_%s_%s_%02d-%02d.xls",
            (int)$up['id'],
    ...
    private static function processSingleJson(\PDO $pdo, int $userId, string $jsonPath, bool $skipDup = false): array {
        ...
        $pdo->prepare("INSERT INTO uploads(user_id, station_id, filename, sha256, period_year, period_month, sat_version, status, errors_json)
                       VALUES(?,?,?,?,?,?,?,?,NULL)")
            ->execute([$userId, $stationId, basename($jsonPath), $sha, $y, $m, $vers, 'warning']);
        ...
        foreach ($prod as $p) {
            ...
            if (isset($rep['ControlDeExistencias']['VolumenExistenciasInicialesMes'])
                ...
                // misma estación por permiso, MISMO usuario
                $exist_ini = self::prevEndingInventoryByPermiso($pdo, $userId, $perm, $pc, $sp, $y, $m);
            }
    ...
    ";
    $st = $pdo->prepare($sql);
    $st->execute([$userId, $permiso, $prevYear, $prevMonth, $product, $subproduct]);
    $val = $st->fetchColumn();

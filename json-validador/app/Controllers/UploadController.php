...
private static function headerNav(string $active = '', ?bool $showDataNav = null): string {
    ...
    private static function assertCsrf(string $t): void {
        ...
        if (!$t || !isset($pool[$t])) { http_response_code(400); die('CSRF inv�lido'); }
    ...
    public static function form(): string {
        ...
        <div class="container">
          <div class="card" style="max-width:760px;margin:0 auto">
            ...
            <p class="muted" style="margin:0 0 16px 0">Puedes seleccionar varios ZIP/JSON a la vez. Agruparemos por estaci�n (RFC + Permiso).</p>
          ...
          <div class="<?= $tab==='monthly'?'':'hidden' ?>">
            <div class="table-wrap" role="region" aria-label="Cargas mensuales" tabindex="0">
              <table>
                ...
                <thead>
                  <tr>
                    <th>ID</th><th>Archivo</th><th>RFC</th><th>Permiso</th><th>Instalaci�n</th>
          ...
          <div class="<?= $tab==='daily'?'':'hidden' ?>">
            <div class="table-wrap" role="region" aria-label="Cargas diarias" tabindex="0">
              <table>
                ...
                <thead>
                  <tr>
                    <th>ID</th><th>Archivo</th><th>RFC</th><th>Permiso</th><th>Instalacion</th>
    ...
    public static function view(): string {
        ...
        if ($id <= 0) { http_response_code(400); return "ID inv�lido"; }
        ...
        <div class="container">
          ...
          <div class="card" style="margin-bottom:16px">
            ...
            <p class="muted">
              ...
              Instalaci�n: <b><?= htmlspecialchars($up['clave_instalacion'], ENT_QUOTES, 'UTF-8') ?></b><br>
              ...
              Versi�n SAT: <b><?= htmlspecialchars((string)$up['sat_version'], ENT_QUOTES, 'UTF-8') ?></b> �
    ...
    public static function handle(): string {
        try {
            ...
            foreach ($fileSpecs as $f) {
                if ($f['err'] !== UPLOAD_ERR_OK) return "Error de subida (c�digo {$f['err']}).";
                if (!is_uploaded_file($f['tmp'])) return "Subida inv�lida.";
                ...
                if ($ext === 'json') {
                    ...
                    return "Extensi�n no permitida (usa .json o .zip).";
                }
            }

            if (!$allJsonPaths) return "No se encontraron JSON v�lidos.";
                    ...
                    <div class="container">
                      <div class="card" style="max-width:780px;margin:0 auto">
                        ...
                        <form method="post" action="/upload-replace" style="display:inline">
                          ...
                          <button class="btn btn-primary" type="submit">S�, reemplazar</button>
                        </form>
            ...
            <div class="container">
              ...
              <div class="table-wrap">
                <table>
                  <thead><tr>
                    <th>Archivo</th><th>RFC</th><th>Permiso</th><th>Instalaci�n</th><th>Versi�n</th>
                    <th>A�o</th><th>Mes</th><th>Compras (L)</th><th>Ventas (L)</th><th># Recepciones</th><th>Status</th>
                  </tr></thead><tbody>
                <?php foreach($rows as $r): ?>
                  <tr>
                    ...
                    <td class="mono"><?= htmlspecialchars((string)($r['a�o'] ?? '')) ?></td>
              ...
              <div style="margin-top:12px">
                ...
                <a href="/upload" class="btn btn-primary">? Subir m�s</a>
                <a href="/" class="btn">Inicio</a>
              </div>
              <div class="footer">� SOPMEX � Validador Anexo 30</div>
            </div>
        ...
        try {
            ...
            <div class="container">
              <div class="card" style="max-width:760px;margin:0 auto">
                ...
                <p>Se reemplaz� la carga anterior por el archivo <b><?= htmlspecialchars($row['archivo'] ?? '') ?></b>.</p>
        ...
        if (($up['report_type'] ?? 'monthly') === 'daily') {
            ...
            <body>
              <table>
                ...
                <tr><td>Upload</td><td><?= (int)$up['id'] ?></td><td>RFC</td><td><?= htmlspecialchars($up['rfc']) ?></td><td>Permiso</td><td><?= htmlspecialchars($up['permiso']) ?></td><td>Instalaci�n</td><td><?= htmlspecialchars($up['clave_instalacion']) ?></td><td>Periodo</td><td><?= (int)$up['period_year'] ?>-<?= str_pad((string)$up['period_month'],2,'0',STR_PAD_LEFT) ?></td></tr>
              </table>
              <br>
              <table>
                <tr>
    ...
    private static function processSingleJson(\PDO $pdo, int $userId, string $jsonPath, bool $skipDup = false): array {
        ...
        if (!is_array($data)) return ["error_archivo"=>basename($jsonPath), "error"=>"JSON inv�lido"];
        ...
        if (!$rfc || !$perm || !$inst) return ["error_archivo"=>basename($jsonPath), "error"=>"Faltan RFC/Permiso/Instalaci�n"];
        ...
        if ($isDaily) {
            ...
            return [
              ...
              "a�o"=>$y, "mes"=>$m, "status"=>"valid", "tipo"=>"daily"
            ];
        }
        ...
        foreach ($prod as $p) {
            ...
            if (isset($rep['ControlDeExistencias']['VolumenExistenciasInicialesMes'])
                ...
                // misma estaci�n (usuario+station_id), s�lo mes anterior exacto
    ...
    private static function unzipJson(string $zipPath, string $outDir): array {
        ...
        $za = new \ZipArchive();
        $ret = $za->open($zipPath);
        if ($ret !== true) throw new \RuntimeException("No se pudo abrir el ZIP (c�digo $ret)");
        @mkdir($outDir, 0770, true);
        if (!$za->extractTo($outDir)) { $za->close(); throw new \RuntimeException("Fall� la extracci�n del ZIP"); }
        $za->close();
    ...
    /** Mes anterior exacto, misma estaci�n (user+station_id), s�lo 'monthly'. */
    ...
    private static function assertZipSupport(): void {
        if (!class_exists('\ZipArchive')) {
            throw new \RuntimeException("Falta la extensi�n php-zip (instala php-zip y recarga Apache).");
        }
    }
    ...
    private static function uploadErrorMessage(int $err): string {
        return match ($err) {
            ...
            default               => "Error de subida (c�digo $err).",
        };
    }
}
